...
def main() -> None:
    ...
    for theme in themes:
        ...
        for exercise in exercises:
            expected = (exercise / "README.md", exercise / "task.py", exercise / "test_task.py")
            missing = [path.name for path in expected if not path.is_file()]
            assert not missing, f"{exercise.relative_to(ROOT)}: отсутствуют {missing}"
            for source in expected[1:]:
                ast.parse(source.read_text(encoding="utf-8"), filename=str(source))
    print(f"Курс корректен: {len(themes)} тем, {sum(len(list(t.glob('[0-9][0-9]_*'))) for t in themes)} заданий")
